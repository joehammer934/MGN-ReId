...
def inference_dataset(loader):
    ...
    with detection_graph.as_default():
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            output = tf.get_default_graph().get_tensor_by_name("result_embedding:0")
            image_tensor = tf.get_default_graph().get_tensor_by_name('image_tensor:0')

            embedding_list = []
            for batch_image, _ in loader:
                image = np.transpose(batch_image.data.numpy(), [0,2,3,1])*255

                output_ = sess.run(output,feed_dict = {image_tensor: image})
                embedding_list.append(output_)
...
if __name__ == '__main__':
    ...
    # mAP = 0.874288, r @ 1 = 0.947150, r @ 3 = 0.975653, r @ 5 = 0.984857, r @ 10 = 0.990499
